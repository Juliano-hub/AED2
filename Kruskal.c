...
int Verifica_Conjunto(int origem, int destino, int *Conjunto);
...
int Verifica_Conjunto(int origem, int destino, int *Conjunto){
    ...
    if(Conjunto[i - 1] != 1){
    //se estiver em uma posição no meio do conjunto
...
void Kruskal(Aresta* Grafo, Aresta* MST, int *Conjunto, int Total_Arestas, int Total_Vertices){
    ...
    for(i=0; i< Total_Arestas; i++){
        if( Verifica_Conjunto( (Grafo + i)->origem, (Grafo + i)->Destino, Conjunto) != 10 ){
